#!/bin/bash
# Learning Rate Sweep for eff_batch=128
#
# Tests 11 learning rates: 5e-5 to 2e-3
# Uses kubernetes schema with small model, tct tokenizer, eff_batch=128, dropout=0.0
# Estimated runtime: ~11 hours overnight
#
# Usage:
#   bash scripts/sweep_lr_eff128.sh          # Fresh run
#   bash scripts/sweep_lr_eff128.sh resume   # Resume from checkpoints
#   bash scripts/sweep_lr_eff128.sh 50       # Run for 50 epochs
...
echo "============================================================"
echo "Learning Rate Sweep (eff_batch=128)"
echo "============================================================"
echo "Date: $(date)"
echo "Schema: $SCHEMA"
...
for lr in 4e-4 5e-4 6e-4 2e-4 1e-4 8e-4 1e-3 1.5e-4 5e-5 1.5e-3 2e-3; do
    ...
    lr_str=$(echo "$lr" | tr -d '-')
    exp_name="sweep_${SCHEMA}_${TOKENIZER}_${MODEL_SIZE}_eff128_lr${lr_str}"
    ...
    echo "[START] LR=$lr (eff_batch=128)"
    echo "Log: $log_file"
...
echo "  grep 'Min val loss' $LOG_DIR/sweep_*_eff128_lr*.log"
echo
