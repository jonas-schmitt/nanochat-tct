...
class EpochOffsetWorkflowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # For INPUT (x): use token 8192 (dedicated PAD token, never produced by TCT)
        ...
        PAD_TOKEN = 8192  # Dedicated padding token (vocab_size = 8193)
        if actual_len < self.context_size:
            # Pad at end with 8192 (dedicated PAD token)
        ...
        # x: [position, tok_0, ..., tok_N-1, PAD, PAD]  (use 8192 for padding - dedicated token)
